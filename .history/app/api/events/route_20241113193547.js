...
export async function POST(request) {
  ...
  form.uploadDir = path.join(process.cwd(), 'public/uploads'); // Save files in the 'public/uploads' directory
  form.keepExtensions = true; // Keep file extensions like .jpg, .png
  ...
  try {
    ...
    const posterPath = files.poster ? `/uploads/${files.poster[0].newFilename}` : '';
    const qrCodePath = files.qrCode ? `/uploads/${files.qrCode[0].newFilename}` : '';

    // Create a new Event document in MongoDB
    const newEvent = new Event({
      eventName: fields.eventName[0],
      location: fields.location[0],
      venueName: fields.venueName[0],
      latitude: fields.latitude[0],
      longitude: fields.longitude[0],
      organizerName: fields.organizerName[0],
      isPaid: fields.isPaid[0] === 'true', // Ensure it's a boolean
      posterName: fields.posterName[0],
      qrCodeName: fields.qrCodeName[0],
