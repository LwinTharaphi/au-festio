...
export async function POST(req) {
  try {
    ...
    const posterPath = posterFile ? await uploadFile(posterFile, 'posters') : null;
    const qrCodePath = qrCodeFile ? await uploadFile(qrCodeFile, 'qrCodes') : null;
    ...
    const newEvent = {
      ...
      latitude,
      longitude,
      poster: posterPath ? fs.readFileSync(posterPath) : null,
      qrCode: qrCodePath ? fs.readFileSync(qrCodePath) : null,
...
async function uploadFile(file, folder) {
  ...
  // Create the path for the file upload
  const filePath = path.join(process.cwd(), 'public', 'uploads', folder, file.name);
  
  // Write file data to the disk (temporary storage example)
  await file.arrayBuffer().then(buffer => fs.writeFileSync(filePath, Buffer.from(buffer)));

  return filePath; // Replace with the actual storage path or URL (for example, cloud storage URL)
}