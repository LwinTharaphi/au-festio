...
export async function GET(request) {
  ...
  const eventsWithPoster = events.map(event => {
    ...
    if (!poster) {
      ...
      // If poster exists, check its structure
      console.log('Poster field:', poster);
    }

    const posterPath = 'D:/SeniorProject/au-festio/public/uploads/posters'+event.posterName;
    console.log('Path : ',posterPath)
    const posterBuffer = fs.readFileSync(posterPath);
    const posterBase64 = poster ? posterBuffer.toString('base64') : null;
    const posterDataUrl = posterBase64 ? `data:image/jpg;base64,${posterBase64}` : null;
    
    console.log('Poster Base64:', posterBase64);
    console.log('Poster Data URL:', posterDataUrl);
