...
function EventForm() {
  ...
  const handleSubmit = async(event) => {
    ...
    } else if (isEditing && !poster) {
      formData.append('poster', currentEvent.posterUrl); // Use the existing file URL when editing
    }
    if (qrCode) {
      formData.append('qrCode', qrCode); // Assuming 'qrCode' is the file data
    } else if (isEditing && !qrCode) {
      formData.append('qrCode', currentEvent.qrCodeUrl); // Use the existing QR code URL when editing
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setIsPaid(false);
        setSelectedEventIndex(null); // Clear selected event index
  ...
  const handleDelete = (index) => {
    ...
    fetch(`/api/events/${eventId}`, {
      ...
      setEvents(events.filter((_, i) => i !== index)); // Remove event from state
