...
function EventForm() {
  ...
  const handleSubmit = () => {
    const newEvent = {
      ...
      organizerName,
      isPaid,
      poster: poster,  // Store poster for the current event
      qrCode: qrCode,  // Store QR code for the current event
      posterName,
      qrCodeName,
    };
    ...
    if (isEditing) {
      ...
      setEvents(updatedEvents); // Update the events list with the modified event
      setIsEditing(false); // Reset edit mode after updating
    } else {
      // Add a new event if not in edit mode
      setEvents((prevEvents) => [...prevEvents, newEvent]); // Append new event to the list
    } 
    // Reset form
    setEventName('');
    setLocation('');
                  ...
                  <CardActionArea>
                    {event.posterName && (
                      <CardMedia 
                        ...
                        image={
                          event.poster
                            ? typeof event.poster === "object" && event.poster !== null
