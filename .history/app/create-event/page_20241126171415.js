...
function EventForm() {
  ...
  const [registerationDate, setRegistrationDate] = useState(new Date().toLocaleString().split('T')[0]);
  ...
  const handleEdit = (index) => {
    ...
    const formattedDate = new Date(eventToEdit.registrationDate).toLocaleString().split('T')[0]; // Converts to YYYY-MM-DD format
